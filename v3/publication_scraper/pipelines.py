...
class PublicationPipeline:
    ...
    def open_spider(self, spider):
        ...
        """
        # Récupérer le dossier de sortie depuis les variables d'environnement
        ...
        if output_format in ['json', 'both']:
            json_path = Path(output_dir) / f'publications_{canton}_{timestamp}.json'
            self.json_file = open(json_path, 'w', encoding='utf-8')
            spider.logger.info(f"Fichier JSON : {json_path}")
        
        if output_format in ['csv', 'both']:
            csv_path = Path(output_dir) / f'publications_{canton}_{timestamp}.csv'

...
class TribunaBern2Spider(scrapy.Spider):
    """
    ...
    def _norm_docid(self, docid: Optional[str]) -> Optional[str]:
        """Normalise un DocId pour les comparaisons internes (case-insensitive)."""
        try:
            if not docid:
                return docid
            return docid.lower()
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        try:
            ...
            strict_flag = False
        spider = super(TribunaBern2Spider, cls).from_crawler(crawler, *args, strict_full=strict_flag, **kwargs)
        try:
    ...
    async def parse_page(self, response):
        ...
        if response.status == 200 and len(response.body) >= MINIMUM_PAGE_LEN:
            ...
            if self.trefferzahl > 0:
                ...
                # Prepare per-page records for concise debug output when running in --days + DEBUG
                try:
                ...
                if len(werte) < 9:
                    ...
                    # Enforce expected items per page detection (UI shows 20 lines/page)
                    expected_per_page = getattr(self, 'EXPECTED_ITEMS_PER_PAGE', 20)
                    limit_ids = min(len(id_indexes), expected_per_page)
                    ...
                    for pos_in_page, idx in enumerate(id_indexes[:limit_ids]):
                        ...
                        # Track unique docids detected on this page (independent of global dedup)
                        try:
                            if doc_id:
                                per_page_detected.add(self._norm_docid(doc_id))
                                # initialize record for concise debug
                                try:
                                    recs = self._page_records.get(current_page, {})
                                    recs.setdefault(self._norm_docid(doc_id), {
                                        'DocIdRaw': doc_id,
                                        'PDatum': '',
                                        'Eligible': False,
                                        'Yielded': False,
                        ...
                        norm_id = self._norm_docid(doc_id)
                        if not doc_id or norm_id in self.seen_ids:
                            drop_counters['dedup_skipped'] += 1
                            logger.debug(f"Dedup skipped raw={doc_id} norm={norm_id}")
                            continue
                        ...
                        if meta is None:
                            ...
                            if getattr(self, 'REQUIRE_ALL_DETECTED_ELIGIBLE', False):
                                ...
                                if isinstance(parsed, dict):
                                    ...
                                    norm_docid = self._norm_docid(docid)
                                    if docid and norm_docid in self._yielded_docids:
                                        next_req = self._barrier_done(current_page, barrier_docid)
                                        if next_req is not None:
                                            yield next_req
                                        continue
                                    if docid:
                                        self._yielded_docids.add(norm_docid)
                        ...
                        # update record PDatum for this docid if present
                        try:
                            if doc_id and current_page in getattr(self, '_page_records', {}):
                                recs = self._page_records.get(current_page, {})
                                rec = recs.get(self._norm_docid(doc_id))
                                if rec is not None:
                                    rec['PDatum'] = pdatum_str or ''
                                    recs[self._norm_docid(doc_id)] = rec
                                    self._page_records[current_page] = recs
                        except Exception:
                            pass
                        ...
                        if getattr(self, 'REQUIRE_ALL_DETECTED_ELIGIBLE', False):
                            eligible_docids += 1
                            # In server-filtered mode, treat detected as eligible
                            page_count_eligible += 1
                            try:
                                # mark record eligible
                                if doc_id and current_page in getattr(self, '_page_records', {}):
                                    recs = self._page_records.get(current_page, {})
                                    rec = recs.get(self._norm_docid(doc_id))
                                    if rec is not None:
                        ...
                        if isinstance(parsed, dict):
                            ...
                            if docid:
                                self._yielded_docids.add(norm_docid)
                                # mark record as yielded
                                try:
                                    if norm_docid and current_page in getattr(self, '_page_records', {}):
                                        recs = self._page_records.get(current_page, {})
                                        rec = recs.get(norm_docid)
                                        if rec is not None:
                                            rec['Yielded'] = True
                                            recs[norm_docid] = rec
                                            self._page_records[current_page] = recs
                    ...
                    # Enforce rule: if the page contains the expected number of unique DocIds
                    # (typical UI page size) but fewer than expected items are eligible,
                    # stop the spider (don't continue to next page).
                    try:
                        expected = getattr(self, 'EXPECTED_ITEMS_PER_PAGE', 20)
                        page_detects_full = len([d for d in per_page_detected if d]) >= expected
                        if page_detects_full and page_count_eligible < expected:
                            logger.info(
                                f"Arrêt: page {current_page} détecte {len(per_page_detected)} DocIds uniques mais seulement {page_count_eligible} éligibles (attendu={expected})"
                            )
                            try:
                                if hasattr(self, 'crawler') and self.crawler and self.crawler.engine:
                                    self.crawler.engine.close_spider(self, reason='page_incomplete_eligibility')
                            except Exception:
                                pass
                    ...
                    # If in --days mode and DEBUG logging requested, print concise per-page report
                    try:
                        log_level = None
                        try:
                        ...
                        if self.min_date is not None and log_level == 'DEBUG':
                            # Print: number of docids, list of (docid, pdatum), eligible vs yielded, per-eligible pdf status
                            total_docids = len(id_indexes)
                            logger.debug(f"DEBUG_SUMMARY page={current_page} total_docids={total_docids}")
                            # List docids + pdatum
                            try:
                                recs = self._page_records.get(current_page, {})
                                for norm, rec in recs.items():
                                ...
                                pass
                            logger.debug(f"DEBUG_SUMMARY page={current_page} eligible={page_count_eligible} yielded={yielded_this_page}")
                    except Exception:
                        pass
        ...
        if response.status == 200:
            ...
            try:
                ...
                try:
                    ...
                    rec = recs.get(self._norm_docid(docid)) if recs and docid else None
                    if rec is not None:
                        rec['Yielded'] = True
                        rec['PDFStatus'] = 'no_pdf'
                        recs[self._norm_docid(docid)] = rec
    ...
    async def verify_pdf_candidate(self, response):
        ...
        if ok:
            ...
            if not self._can_emit_more():
                return
            # Prevent duplicate yields by DocId (normalize for internal dedup)
            docid = item.get('DocId')
            norm_docid = self._norm_docid(docid)
            if docid and norm_docid in self._yielded_docids:
                return
            ...
            try:
                if item and page_nr is not None and hasattr(self, '_page_records'):
                    ...
                    rec = recs.get(self._norm_docid(item.get('DocId'))) if recs and item.get('DocId') else None
                    if rec is not None:
                        # check if file already exists in output/pdfs (match by filename)
                        try:
                            ...
                            rec['Yielded'] = True
                            recs[self._norm_docid(item.get('DocId'))] = rec
                            self._page_records[page_nr] = recs
                        except Exception:
                            rec['PDFStatus'] = 'found'
                            rec['Yielded'] = True
        ...
        # If not OK, mark PDF failure in page records when possible
        try:
            if item and page_nr is not None and hasattr(self, '_page_records'):
                recs = self._page_records.get(page_nr, {})
                rec = recs.get(self._norm_docid(item.get('DocId'))) if recs and item.get('DocId') else None
                if rec is not None:
                    # if ok==False but candidates remain, we'll try next; otherwise mark failed
                    if not candidates:
        ...
        if not self._can_emit_more():
            return
        # Prevent duplicate yields (normalisation interne)
        docid = item.get('DocId')
        norm_docid = self._norm_docid(docid)
        if docid and norm_docid in self._yielded_docids:
            return

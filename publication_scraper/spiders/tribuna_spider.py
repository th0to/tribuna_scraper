...
from publication_scraper.lib import GWTTokenManager, PDFDecryptor, DateDetector, PageBarrier
...
class TribunaFribourgSpider(scrapy.Spider):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # --- lib/ services (incremental refactor) ---
        # These helpers encapsulate the core mechanics (tokens/dates/pdf/barrier)
        # while the spider remains the Scrapy orchestrator.
        self._gwt_manager = GWTTokenManager(self.config, self.HEADERS)
        self._pdf_decryptor = PDFDecryptor(self.config)
        try:
        ...
        self._date_detector = DateDetector(
            pdatum_future_days=pd_future_days,
            edatum_future_days=ed_future_days,
        ...
        self._page_barrier = PageBarrier(enabled=bool(self.PAGE_PDF_BARRIER))
    ...
    def _barrier_register(self, page_nr: int, docid: Optional[str]) -> None:
        ...
        if not getattr(self, 'PAGE_PDF_BARRIER', False):
            return
        try:
            self._page_barrier.register(page_nr, docid)
        except Exception:
            return
    ...
    def _barrier_done(self, page_nr: int, docid: Optional[str]):
        ...
        if not getattr(self, 'PAGE_PDF_BARRIER', False):
            return None
        try:
            return self._page_barrier.done(page_nr, docid)
        except Exception:
            return None
    ...
    def _detect_pdf_path(
        ...
        # Primary detection: delegate token scan to lib/pdf_decryptor.
        try:
            pfad, neuePfadsyntax = self._pdf_decryptor.detect_pdf_path(tokens)
        except Exception:
            pfad, neuePfadsyntax = None, False

        # Preserve existing raw-content fallback semantics (DocId-local window scan).
    ...
    def _detect_dates(
        ...
        *,
        ...
        if pick_mode in {'last', 'last_in_row', 'row_last', 'tail'}:
            try:
                # Delegate the "last_in_row" pick to lib/date_detector to keep this
                # method focused on orchestration + fallbacks/tracing.
                try:
                    row_end_for_pick = int(l)
                except Exception:
                    row_end_for_pick = int(row_end)
                det_ed, det_pd = self._date_detector.detect_dates(
                    tokens,
                    id_pos,
                    row_end_hint=row_end_for_pick,
                    doc_id=doc_id,
                )
                if det_pd:
                    if not (entscheiddatum and det_pd == entscheiddatum):
                        publikationsdatum = det_pd
                        picked_by_mode = True
            except Exception:
                picked_by_mode = False
    ...
    async def parse_bootstrap_nocache(self, response: Response):
        ...
        try:
            self._gwt_manager.extract_tokens_from_nocache(response)
        except Exception:
            pass
        try:
            self._bootstrap_perm_found = bool(getattr(self._gwt_manager, '_bootstrap_perm_found', False))
        except Exception:
    ...
    def _update_gwt_tokens(self, response: Response) -> List[str]:
        ...
        try:
            nocache_urls = self._gwt_manager.extract_tokens_from_html(response)
        except Exception:
            nocache_urls = []
        try:
            self._bootstrap_perm_found = bool(getattr(self._gwt_manager, '_bootstrap_perm_found', False))
            ...
            self._bootstrap_perm_found = False
        if self._bootstrap_perm_found:
            # Reset bootstrap retry counter once we lock a permutation
            self._bootstrap_retry_count = 0

        # Preserve the richer warning with bootstrap snippet when nothing was found.
        if (not self._bootstrap_perm_found) and (not nocache_urls):
            try:
        ...
        if len(werte) < 9:
            ...
            if not stop_due_to_date and should_continue:
                ...
                if getattr(self, 'PAGE_PDF_BARRIER', False):
                    try:
                        stored = self._page_barrier.store_next_request(current_page, next_req)
                    except Exception:
                        stored = False
                    if stored:
                        try:
                            pending_n = self._page_barrier.get_pending_count(current_page)
                        except Exception:
                            pending_n = 0
                        logger.info(
                            f"Barrier active: attente PDFs page {current_page} (pending={pending_n}), pagination différée"
                        )
                    else:
                        yield next_req
                else:
        ...
        try:
            ...
            if self.ENCRYPTED:
                ...
                if self.ASCII_ENCRYPTED:
                    ...
                    # Delegate the canton-specific string building to lib/pdf_decryptor.
                    body = self._pdf_decryptor.build_decrypt_body(pfad, numstr, neuePfadsyntax)
    ...
    async def decrypt_path(self, response):
        ...
        if response.status == 200:
            ...
            if not built:
                try:
                    built = self._pdf_decryptor.build_pdf_candidates(response.text, dict(item))
                except Exception:
                    built = None
            if built:

...
MAX_IDS_FOR_HINT_COMPUTATION = 50  # Nombre max de DocIds à traiter pour le hint
...
class FribourgSpider(scrapy.Spider):
    ...
    def _search_pdatum_in_content(
        ...
        2. Score = (is_before, -date_numeric, distance)
           - Préférer dates APRÈS le DocId (is_before=0 meilleur que is_before=1)
           - Puis la date la plus récente (-date descending)
        ...
        # Fribourg: fenêtre 3000 chars avant le DocId, jusqu'au prochain DocId
        start = max(0, doc_id_pos - 3000)
        max_window = 22000
        ...
        for pos, iso in matches_with_pos:
            ...
            # Score: (is_before, -date_numeric, distance)
            # Plus petit = meilleur (is_before=0 > is_before=1, date plus récente, distance plus courte)
            date_numeric = int(d.strftime('%Y%m%d'))
            score = (is_before, -date_numeric, dist)
    ...
    async def parse_page(self, response: Response):
        ...
        # Calculer le pdatum_hint pour chaque DocId (propagation marker)
        ...
        for i, idx in enumerate(id_indexes[:limit_ids]):
            ...
            # Récupérer le pdatum_hint pour ce DocId
            pdatum_hint = pdatum_hint_map.get(doc_id, '') or None
            
            # Détecter les dates avec la logique complète
        ...
        logger.info(f"Page {current_page}: {eligible_count} éligibles, {older_count} trop anciens")

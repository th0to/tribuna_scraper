...
class TribunaBase(scrapy.Spider):
    """
    ...
    """
    # Base spider class: concrete spiders must define a unique `name`.
    ...
    DEFAULT_CANTON: Optional[str] = None
    ...
    def _stats_inc(self, key: str, count: int = 1) -> None:
        """Increment a Scrapy stats counter safely.

        Scrapy always sets `self.crawler` at runtime, but static analyzers may treat it as optional.
        This helper keeps the callsites clean and makes failures non-fatal.
        """
        try:
        ...
        if self.max_date:
            try:
                if publikationsdatum:
                    pd_dt = self._parse_date_any(publikationsdatum)
                    if pd_dt is not None and pd_dt > self.max_date:
                        publikationsdatum = ''
                if entscheiddatum:
                    ed_dt = self._parse_date_any(entscheiddatum)
                    if ed_dt is not None and ed_dt > self.max_date:
                        entscheiddatum = ''
            except Exception:
                pass
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        try:
            ...
            strict_flag = False
        spider = super(TribunaBase, cls).from_crawler(crawler, *args, strict_full=strict_flag, **kwargs)
        try:
    ...
    async def start(self):
        ...
        try:
            if hasattr(self, 'crawler') and self.crawler and self.crawler.settings:
                # Scrapy Settings.getint uses `default`, not `fallback` (Scrapy 2.13.3)
        ...
        try:
            _min_date = self.min_date.isoformat() if self.min_date else None
        except Exception:
            _min_date = None
        try:
            _max_date = self.max_date.isoformat() if self.max_date else None
        except Exception:
            _max_date = None
        try:
            logger.info(
    ...
    def _update_gwt_tokens(self, response: Response) -> List[str]:
        ...
        cache_scripts: List[str] = []
        nocache_scripts: List[str] = []
        try:
        ...
        def _module_from_src(src_url: str) -> Optional[str]:
            """Prefer Scrapy's URL join semantics when response is available."""
            try:
            ...
            if not base_part:
                return None
            try:
                return response.urljoin(base_part)
            except Exception:
                try:
                    return urljoin(base_url, base_part)
                except Exception:
                    return None
        ...
        # Note: on ne fetch PAS nocache ici; on retourne les URLs candidates pour que
        # parse_bootstrap/parse_bootstrap_module puissent les suivre via Scrapy.

        # Module fallback: infer from first cache script if still missing
        ...
        try:
            return [response.urljoin(nc) for nc in (nocache_scripts or [])]
        except Exception:
            try:
                return [urljoin(base_url, nc) for nc in (nocache_scripts or [])]
            except Exception:
                return []
    ...
    async def parse_page(self, response):
        ...
        # Always define this for the whole method scope (used later in pagination heuristics).
                ...
                if len(werte) < 9:
                    ...
                    if id_indexes and len(id_indexes) > max(80, int(getattr(self, 'max_ids_per_page', 25)) * 3):
                        try:
                            ...
                            for pos_in_page, idx in enumerate(id_indexes):
                                ...
                                num = str((meta.get('Num') or '') if isinstance(meta, dict) else '').strip()

                                # Prefer PDatum, fallback EDatum for sorting.
                                pds = str((meta.get('PDatum') or '') if isinstance(meta, dict) else '').strip()
                                eds = str((meta.get('EDatum') or '') if isinstance(meta, dict) else '').strip()
                                dt = None
                                try:
                                    if pds:
                    ...
                    for pos_in_page, idx in enumerate(id_indexes[:limit_ids]):
                        ...
                        if meta is not None and self.min_date:
                            pdatum_try = str((meta.get('PDatum') or '') if isinstance(meta, dict) else '').strip()
                            pdate_obj_try = None
                            if pdatum_try:
                                try:
                        ...
                        if meta is None:
                            ...
                            if getattr(self, 'REQUIRE_ALL_DETECTED_ELIGIBLE', False):
                                ...
                                try:
                                    ...
                                    barrier_docid = doc_id
                                if not isinstance(barrier_docid, str) or not barrier_docid:
                                    barrier_docid = doc_id
                        ...
                        pdatum_str = str((meta.get('PDatum') or '') if isinstance(meta, dict) else '').strip()
                        edatum_str = str((meta.get('EDatum') or '') if isinstance(meta, dict) else '').strip()
                        pdatum_obj = None
                        edatum_obj = None
                        if pdatum_str:
                            try:
                        ...
                        if self.min_date and cutoff_date_obj and cutoff_date_obj < self.min_date:
                            is_eligible = False
                        if self.max_date and cutoff_date_obj and cutoff_date_obj > self.max_date:
                            is_eligible = False
